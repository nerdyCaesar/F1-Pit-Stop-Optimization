...
# Project Root and Subdirectory Setup (from src/ to root)
...
TRAINING_CSV = DATA_DIR / "f1_lap_data.csv"
...
def load_training_data():
    ...
    try:
        ...
        print(f"\n [CRITICAL ERROR] Missing processed dataset in '{DATA_DIR}'.")
...
def run_step_1_kfold(X, y, groups, feature_cols, n_splits=5):
    ...
    # PR-AUC calculations
    base_rate = y.mean()
    prauc_all = average_precision_score(y, oof_probabilities)
    ...
    plt.ylabel('Actual')
    plt.tight_layout()
    plt.savefig(OUTPUTS_DIR / 'confusion_matrix_kfold.png')
    plt.close()
    ...
    joblib.dump(final_clf, MODELS_DIR / "final_model.pkl")

    metrics = {
        "model_type": type(final_clf).__name__,
        "threshold": PROBABILITY_THRESHOLD,
        "pr_auc": float(average_precision_score(y, oof_probabilities)),
    ...
    # Save Feature Importance Diagram
    ...
    print(f" -> Saved 'final_model.pkl' to {MODELS_DIR}")
    print(f" -> Saved metrics and diagrams to {OUTPUTS_DIR}")
    return final_clf
...
def demonstrate_race_predictions(clf, df, feature_cols, target_race_name="Abu Dhabi Grand Prix", target_year=2024):
    ...
    output_filename = OUTPUTS_DIR / f"race_demo_{target_year}_{target_race_name.replace(' ', '_')}.csv"
